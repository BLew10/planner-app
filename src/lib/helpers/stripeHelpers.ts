...
export async function getStripeCustomerById(id: string): Promise<Stripe.Customer | Stripe.DeletedCustomer | null> {
    try {
...
export async function getStripePriceById(id: string): Promise<Stripe.Price | null> {
    try {
  ...
  scheduleId: string
    ...
    try {
      ...
      if (oldPrice?.recurring?.interval !== interval) {
        ...
        const newPrice = await createStripePrice(amountInCents, frequency, stripePriceName);
        if (!newPrice) {
            return null
        }
...
const stripeIntervalMap = (frequency: string): Stripe.SubscriptionScheduleCreateParams.Phase.Item.PriceData.Recurring.Interval => {
  switch (frequency) {
    case "Daily":
      return "day";
    case "Weekly":
...
export async function updateStripeSubscriptionSchedule(
  ...
  startDate: string
    ...
    try {
        ...
        const isToday =
            new Date(startDateFormat).toDateString() === new Date().toDateString();
        const scheduleStartDate = isToday ? "now" : Math.floor(new Date(startDateFormat).getTime() / 1000);
...
export async function cancelStripeSchedule(scheduleId: string): Promise<Stripe.SubscriptionSchedule | null> {
