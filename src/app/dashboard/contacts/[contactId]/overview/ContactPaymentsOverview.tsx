...
const defaultYear = ALL_YEARS.find((year) => year.value === (new Date().getFullYear() + 1).toString())?.value || ALL_YEARS[0].value;

const ContactPaymentsOverview = ({ contactId }: ContactPaymentsOverviewProps) => {
      ...
      <TableBody>
        {payments?.map((payment, index) => (
          <TableRow key={index}>
            ...
            <TableCell>{payment.paymentOverview?.invoiceNumber || "—"}</TableCell>
            <TableCell>{payment.purchase?.calendarEditions?.map((c) => c.code).join(", ") || "—"}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
      ...
      {payments?.map((payment, index) => (
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-500 dark:text-gray-400">Check Number</span>
              <span>{payment.checkNumber || "—"}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-500 dark:text-gray-400">Invoice Number</span>
              <span>{payment.paymentOverview?.invoiceNumber || "—"}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-500 dark:text-gray-400">Calendar Editions</span>
              <span>{payment.purchase?.calendarEditions?.map((c) => c.code).join(", ") || "—"}</span>
            </div>
          </div>
        </div>
      ))}
  ...
  return (
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          viewMode === 'card' ? renderCardLoadingState() : renderTableLoadingState()
